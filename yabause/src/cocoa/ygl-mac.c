...
#ifndef __gl3_h_
#error This file requires OpenGL 3 or later!
#endif


#if defined(DEBUG) && DEBUG
#define DEBUGGL(__x) do { \
__x; \
error = glGetError();\
if (error != 0) { \
printf( "%s:%i " #__x "! error 0x%04x\n", __FILE__, __LINE__, error);\
 } \
} while(0)

#else
#define DEBUGGL(__x) __x
#endif
...
int YglGLInit(int width, int height) {
   ...
   DEBUGGL(glClear(GL_COLOR_BUFFER_BIT|GL_DEPTH_BUFFER_BIT));
   ...
   DEBUGGL(glEnable(GL_BLEND));
   DEBUGGL(glBlendFunc(GL_SRC_ALPHA, GL_ONE_MINUS_SRC_ALPHA));

   DEBUGGL(glDisable(GL_DEPTH_TEST));
   DEBUGGL(glDepthFunc(GL_GEQUAL));
   DEBUGGL(glClearDepthf(0.0f));
   
   DEBUGGL(glCullFace(GL_FRONT_AND_BACK));
   DEBUGGL(glDisable(GL_CULL_FACE));
   DEBUGGL(glDisable(GL_DITHER));

   error = glGetError();

   DEBUGGL(glPixelStorei(GL_PACK_ALIGNMENT, 1));
   DEBUGGL(glPixelStorei(GL_UNPACK_ALIGNMENT, 1));

   YGLLOG("YglGLInit(%d,%d)\n",GlWidth,GlHeight );

   if( _Ygl->texture == 0 )
      DEBUGGL(glGenTextures(1, &_Ygl->texture));

  DEBUGGL(glGenBuffers(1, &_Ygl->pixelBufferID));
  DEBUGGL(glBindBuffer(GL_PIXEL_UNPACK_BUFFER, _Ygl->pixelBufferID));
  DEBUGGL(glBufferData(GL_PIXEL_UNPACK_BUFFER, width * height * 4, NULL, GL_STREAM_DRAW));
  DEBUGGL(glBindBuffer(GL_PIXEL_UNPACK_BUFFER, 0));
   ...
   DEBUGGL(glTexParameteri( GL_TEXTURE_2D, GL_TEXTURE_MIN_FILTER, GL_NEAREST ));
   DEBUGGL(glTexParameteri( GL_TEXTURE_2D, GL_TEXTURE_MAG_FILTER, GL_NEAREST ));
   DEBUGGL(glTexParameteri(GL_TEXTURE_2D, GL_TEXTURE_WRAP_S, GL_CLAMP_TO_EDGE));
   DEBUGGL(glTexParameteri(GL_TEXTURE_2D, GL_TEXTURE_WRAP_T, GL_CLAMP_TO_EDGE));
   ...
   DEBUGGL(glDeleteTextures(2,_Ygl->vdp1FrameBuff));
   DEBUGGL(glGenTextures(2,_Ygl->vdp1FrameBuff));
   DEBUGGL(glBindTexture(GL_TEXTURE_2D,_Ygl->vdp1FrameBuff[0]));
   DEBUGGL(glTexImage2D(GL_TEXTURE_2D, 0, GL_RGBA, GlWidth, GlHeight, 0, GL_RGBA, GL_UNSIGNED_BYTE,NULL));
   DEBUGGL(glTexParameteri( GL_TEXTURE_2D, GL_TEXTURE_MIN_FILTER, GL_NEAREST ));
   DEBUGGL(glTexParameteri( GL_TEXTURE_2D, GL_TEXTURE_MAG_FILTER, GL_NEAREST ));
   DEBUGGL(glTexParameteri(GL_TEXTURE_2D, GL_TEXTURE_WRAP_S, GL_CLAMP_TO_EDGE));
   DEBUGGL(glTexParameteri(GL_TEXTURE_2D, GL_TEXTURE_WRAP_T, GL_CLAMP_TO_EDGE));

   DEBUGGL(glBindTexture(GL_TEXTURE_2D,_Ygl->vdp1FrameBuff[1]));
   DEBUGGL(glTexImage2D(GL_TEXTURE_2D, 0, GL_RGBA, GlWidth, GlHeight, 0, GL_RGBA, GL_UNSIGNED_BYTE,NULL));
   DEBUGGL(glTexParameteri( GL_TEXTURE_2D, GL_TEXTURE_MIN_FILTER, GL_NEAREST ));
   DEBUGGL(glTexParameteri( GL_TEXTURE_2D, GL_TEXTURE_MAG_FILTER, GL_NEAREST ));
   DEBUGGL(glTexParameteri(GL_TEXTURE_2D, GL_TEXTURE_WRAP_S, GL_CLAMP_TO_EDGE));
   ...
   {
      if( _Ygl->rboid_depth != 0 ) DEBUGGL(glDeleteRenderbuffers(1,&_Ygl->rboid_depth));
      DEBUGGL(glGenRenderbuffers(1, &_Ygl->rboid_depth));
      DEBUGGL(glBindRenderbuffer(GL_RENDERBUFFER,_Ygl->rboid_depth));
      DEBUGGL(glRenderbufferStorage(GL_RENDERBUFFER,  GL_DEPTH24_STENCIL8, GlWidth, GlHeight));
      ...
      DEBUGGL(glGenRenderbuffers(1, &_Ygl->rboid_depth));
      DEBUGGL(glBindRenderbuffer(GL_RENDERBUFFER,_Ygl->rboid_depth));
      DEBUGGL(glRenderbufferStorage(GL_RENDERBUFFER,  GL_DEPTH_COMPONENT16, GlWidth, GlHeight));

      if( _Ygl->rboid_stencil != 0 ) DEBUGGL(glDeleteRenderbuffers(1,&_Ygl->rboid_stencil));
      DEBUGGL(glGenRenderbuffers(1, &_Ygl->rboid_stencil));
      DEBUGGL(glBindRenderbuffer(GL_RENDERBUFFER,_Ygl->rboid_stencil));
      DEBUGGL(glRenderbufferStorage(GL_RENDERBUFFER,  GL_STENCIL_INDEX8, GlWidth, GlHeight));
   }


   DEBUGGL(glBindFramebuffer(GL_FRAMEBUFFER, _Ygl->vdp1fbo));
   DEBUGGL(glFramebufferTexture2D(GL_FRAMEBUFFER, GL_COLOR_ATTACHMENT0, GL_TEXTURE_2D, _Ygl->vdp1FrameBuff[0], 0));
   ...
   DEBUGGL(status = glCheckFramebufferStatus(GL_FRAMEBUFFER));
...
int YglInit(int width, int height, unsigned int depth) {
   ...
   DEBUGGL(glGenFramebuffers(1,&_Ygl->vdp1fbo));
   DEBUGGL(glBindFramebuffer(GL_FRAMEBUFFER, _Ygl->vdp1fbo));
   DEBUGGL(glFramebufferTexture2D(GL_FRAMEBUFFER, GL_COLOR_ATTACHMENT0, GL_TEXTURE_2D, _Ygl->vdp1FrameBuff[0], 0));
   DEBUGGL(glFramebufferRenderbuffer(GL_FRAMEBUFFER, GL_DEPTH_ATTACHMENT, GL_RENDERBUFFER, _Ygl->rboid_depth));
   DEBUGGL(glFramebufferRenderbuffer(GL_FRAMEBUFFER, GL_STENCIL_ATTACHMENT, GL_RENDERBUFFER, _Ygl->rboid_stencil));
